...
普通 frozen 的适用范围已获用户按 `reassessment@3e41130` 批准：`1.4b/4.5b` 从平台 7.2 实际 EXE 消费既有 composition 与真实 ProjectPackage，不依赖 W3 源码证明或平台 7.4 最终汇总。数据保护和业务合同不变；候选必测、未变 owner 深入矩阵复用及变更触发重验遵循平台 Requirement 12，不把 source evidence 改签为 packaged 结果。
