...
- **Route staging**：source状态保留原完成事实。平台7.2a与Core 9.6e提供真实输入/worker后，3.6b贯通Host并供平台7.2b/7.2c整合；6.6b随后消费Core 9.6b形成资格UI闭环，7.4b/7.6b/9.2a在平台7.4最终汇总前完成自身候选验收。任何owner均不等待平台7.4先通过，也不以W3 spike为普通前置。

普通Host继续采用[Core publication合同](../tm-storage-retrieval-index/trusted-input-publication.md)的原子性：Gate C与Gate D沿同一publisher/lifecycle reservation，完整handoff、display、generation、notification与owner success在commit前预构造，锁内只安装原引用。取消先赢则零安装；合法commit先赢则保留完整安装和完成事实。所谓禁止晚到发布仅针对已被撤销的请求，不能把已提交结果事后倒判失败。Qt queued通知只传播已经提交的generation，旧通知不得使新请求获得资格；child停止/等待/句柄回收在短锁外并由异步结束事实单独闭合。

设备资格继续由Core执行Matcher/Gate C后恢复ADR-013 attestation；失败只关闭Fuzzy，Exact/Context保留各自Gate，不自动100k。Core兼容身份覆盖检索相关实现/runtime/计量变化，Host不将候选ID或全部EXE/PYZ摘要塞进资格key；纯UI/头像变化只触发受影响UI/资源回归。显式重验始终调用原Core真实runner与same-EXE worker，UI的`RUNNING/SUCCEEDED`仅作显示。source/W3资格不能未经Core兼容/provenance验证直接用于普通packaged。

本节不要求旁置源码或`proof-inputs`目录；普通Host只消费Core明确定义的有限输入和真实端口。若实际组合仍依赖源码AST，3.6b须定位到具体binding并在该接缝解除依赖，不扩大为新authority体系。下一实验通过同一真实候选的首页/项目→Host→Matcher/Gate C→小样本worker结果→通知/取消/退出链，核实是否仍有ambient源码读取、主线程等待环或child残留；小样本不签发正式资格。
