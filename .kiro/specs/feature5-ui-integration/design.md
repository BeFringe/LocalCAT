...
- **Frozen pre-build接入**：2026-09-19批准的WA-06 R4由Core 9.6c/9.6d交付共同受信输入session和fresh worker消费，3.6a以此建立consumer implementation checkpoint；实际native producer归平台7.2，同候选runtime仍归6.6b/7.4b/7.6b/9.2a。WA-07保持R1，不新增capability issuer或public schema。
- **后台proof window与撤销**：native take/read/reproof/close保持原owner thread/interpreter串行执行，Feature5后台Gate只取得有效session内的bytes/attestation，不取得native对象。WA-07在Boot TCB闭合后注入Qt调度适配，平台authority不依赖Qt；等待、取消、close、异常及重入必须唤醒等待者并撤销未完成session，UI主线程不得反向等待依赖它的worker。发布前仍需fresh terminal reproof，缓存或排队旧证明不能绕过撤销；source route继续回归。
