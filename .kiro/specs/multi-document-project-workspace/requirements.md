...
11. When 当前单个 Document 的段落数按设备本地分组设置超过组数阈值 **或** 段数阈值且分组已启用时, Browse/Review shall 在主页表格左侧显示当前文件的可垂直滚动分组轮次导航，并压缩表格的“段落”列；用户可选择默认的“自动收起式”窄轮次指示条或“固定式”预览列表，两者必须共享同一 current group 与跳转状态。标题行可保留一个“轮次 n / N”按钮，但其弹窗只包含显示方式与分组设置。默认每组 20 段、超过 5 组或 100 段显示；每组只能以 10 段递增调整为 20–200 段；当前文档变化时，轮次导航立即重建为新文档的分组与当前组。
