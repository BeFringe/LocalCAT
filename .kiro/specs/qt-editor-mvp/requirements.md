...
7. When 用户在新建资源对话框选择翻译记忆库或术语表, the 语言资源设置 shall 创建所选类型而不因界面数据表示差异拒绝有效选择
8. When 用户调整语言资源设置窗口尺寸, the 语言资源设置 shall 保持资源类型和导入操作文字完整可见，并让名称与路径列利用新增空间
...
### Requirement 9：桌面启动与项目生命周期

**目标：** 作为持续翻译长篇项目的个人译者，我希望像使用 IDE 或记事本一样启动、切换和恢复项目，以便不依赖每次重新定位。

#### 验收标准

1. When 用户安装桌面启动入口, the LocalCAT Qt 编辑器 shall 可从桌面环境的应用菜单启动且不要求用户手动切换到仓库目录
2. When 用户成功打开或保存一个本地项目, the LocalCAT Qt 编辑器 shall 将该项目加入本地最近项目列表并把最新项目排在最前
3. When 用户在项目内切换段落, the LocalCAT Qt 编辑器 shall 在本地记录该项目最后访问的段落
4. When 用户再次打开最近项目, the LocalCAT Qt 编辑器 shall 恢复到上次记录的有效段落，并在记录失效时安全回到首段
5. When 用户选择打开其他项目或退出当前项目, the LocalCAT Qt 编辑器 shall 先执行未保存保护，再切换项目或返回无项目空状态
6. If 最近项目文件已不存在或无法读取, the LocalCAT Qt 编辑器 shall 显示可理解的错误并允许用户继续使用其他项目
7. When 用户选择退出 LocalCAT, the LocalCAT Qt 编辑器 shall 执行与窗口关闭相同的未保存保护

### Requirement 10：段落密度与浏览校对

**目标：** 作为处理长篇文本的个人译者，我希望在高密度导航、完整段落浏览和当前段编辑之间切换，以便兼顾进度定位与全文校对。

#### 验收标准

1. While 段落列表处于紧凑模式, the LocalCAT Qt 编辑器 shall 以稳定等高单行展示段号、确认状态和源文摘要，并通过悬停显示完整源文
2. While 段落列表处于自动换行模式, the LocalCAT Qt 编辑器 shall 展示完整源文并根据可用栏宽调整每行高度
3. When 用户切换紧凑模式与自动换行模式, the LocalCAT Qt 编辑器 shall 保持当前项目、当前段、译文编辑和确认状态不变
4. When 用户切换到浏览校对模式, the LocalCAT Qt 编辑器 shall 以可滚动的双语行同时展示源文、译文和确认状态，并对长文本自动换行
