...
- [x] 14. 补齐协作分工及遗漏窗口的动态深浅主题与导出反馈
  - 排查全部 Qt 局部 QSS、palette、viewport、popup 和自绘颜色；包括分工管理、TMX 导出、项目包与资源包导入、新建资源、术语管理与浏览分组
...
- 项目包导入预览的具名浅色选择器优先于通用深色规则，需要在现有深色 QSS 中逐一覆盖；主题切换不得改变预览、项目 ID 选区、默认取消或 Apply 禁用状态。
...
- 回归后的测量口径、机制定位及历史样本限制见 [启动优化复测对照](startup-comparison.md)；该调查索引不补充设计授权或代表 Task 16 完成。
