...
- session 自行定义唯一合法精化集 `R = {未计入 r | U1(r) >= threshold 或 (U1(r), record_id(r)) >= K0}`。phase 2 必须重新绑定同一 resource/store/generation/head/count/query/index facts，在一个短只读事务内仅为严格有序的 `R` 返回 store-owned `record_id/source_fold_v1/length` 私有投影，拒绝缺失、重复、乱序、越界或额外 identity，并在提交后计算 folded Unicode code-point 序列的精确 LCS 长度 `ell`。令 `d3=max(abs(m-n), L-ell, ceil((Bq+Br-2I)/4))`，据此得到 `U3` 并继续真实 scorer-v1；因为 `ell<=C<=C+`，必须以穷举/随机对照证明 `真实分数 <= U3 <= U2 <= U1`。LCS 只产生 Levenshtein 距离下界，不得计算编辑距离、final score 或建立 scorer 等价类；后者仍由 Retrieval 从 health-validated raw record 重新 fold 后独占。两个事务均不得跨 scorer callback；phase 2 前、phase 2 中/后及 scorer 期间的 append/head 漂移分别由 phase binding 或最终 generation/head 复核稳定 fail-closed。
- 两阶段证明冻结 `A0`（精化前已计入）、`P1`（U1 安全排除）、`R`、`A1`（R 中后续计入）与 `P2`（U3 安全排除），并强制 `total=A0+P1+R`、`R=A1+P2`、`accounted=A0+A1`、`unscored=P1+P2`。最终 threshold 与 top-k 闭合必须同时证明 `P1` 的最大 U1 前沿和 `P2` 的最大 U3 前沿均被真实 threshold 与最终 kth 元组严格支配；相等仍视为未闭合。公开 metadata 只携带 ordered-bound/traversal phase/version、是否精化、各分区计数/最大前沿、精化请求与返回计数及 `K0`，不得泄露 folded text、LCS、gram、等价键或正文。
