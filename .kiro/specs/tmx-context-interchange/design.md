...
- **ADR mapping**：follow ADR-020/025/028。TMX 保留 payload grammar、scope/loss、canonical writer 与 direct receipt；Parser 提供 sealed source，platform 提供 `BoundDirectoryPublisher` 与普通 bundle 布局。普通来源和 Task 5.4a 已获用户按 `reassessment@3e41130` 明确批准：实际执行的模块及必要 fixture/data 来自同一候选，不要求 Parser `.py` 或 W3 native entry；按平台 Requirement 12 进行候选必测、未变证据复用及变更重验，source 原范围不变。
