...
`W1`～`W3` 是本 Spec 使用的 **Windows ADR candidate working labels**。用户已在 2026-08-25 将三者批准为 `APPROVED_FOR_BASELINE_NAMING`：可作为稳定候选引用、依赖图和派发标识；治理分支现已将其唯一映射为 ADR-020/W1、ADR-021/W2、ADR-022/W3 草案。该命名和草案状态都不等于正式 ADR 已采纳，也不授权平台实现；只有人工采纳及治理提交后，正式 ADR 编号才具有约束力。
...
| **W2** | **Windows Device-local Private Attestation Representation** | 在 Windows 物理表示范围部分取代 ADR-013/016 的 POSIX uid/mode/dev/inode 谓词，冻结 `windows-private-v1` SID/DACL/AccessCheck 与嵌套 `WindowsPrivateProof`；Gate D/canonical owner envelope 保持正交。 | 依赖 W1 的 identity/error contracts；仅 WA-06/07 直接依赖。参考 [design.md § Private Storage Proof](design.md#private-storage-proof)、[ADR-013](../../steering/adr/adr-013.md)、[ADR-016](../../steering/adr/adr-016.md)。 |
...
- 派发包必须附 publish mode/threat scope、retained readback→owner commit→terminal reproof、匹配 `DurabilityProfile` 的 power-cut success boundary、owner-specific envelope + nested W2 proof 与 recovery results。
