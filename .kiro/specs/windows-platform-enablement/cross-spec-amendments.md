...
ADR-020～023 promotion、Windows owning scope、WA-01～08 current R/D/T delta及owning Spec acknowledgement已批准。WA-06 `R1`因V2 security profile增量被`R2` request取代；amendment commit、merge identity、evidence和终态disposition仍待后续implementation clusters逐行闭合。任一`CONTRACT_CHANGE`行缺少current approved R/D/T、commit或可达merge时，对应Windows consumer cluster与最终EXE gate均为 **NO-GO**。
...
|---|---|---|---|
| WA-01 | `parser-subsystem-extraction` | Tasks：source/frozen completion拆分及candidate依赖 | `APPROVED 2026-08-26` |
| WA-02 | `collaborative-job-chunks` | Tasks：source/frozen composition/acceptance拆分及candidate依赖 | `APPROVED 2026-08-26` |
| WA-03 | `multi-document-project-workspace` | ledger-only：source owner结果由WA-08 packaged Project journey复验；owner R/D/T不改 | `APPROVED 2026-08-26` |
| WA-04 | `language-resource-portability` | Requirements/Tasks：source authority与packaged resource revalidation拆分 | `APPROVED 2026-08-26` |
| WA-05 | `tmx-context-interchange` | Tasks：source TMX completion与packaged journey拆分 | `APPROVED 2026-08-26` |
| WA-06 | `tm-storage-retrieval-index` | Tasks：source TM completion与candidate frozen revalidation拆分 | `APPROVED 2026-08-26` |
| WA-07 | `feature5-ui-integration` | Requirements/Design/Tasks：source/frozen authority、pre-build bootstrap消费与post-build GO拆分 | `APPROVED 2026-08-26` |
| WA-08 | `qt-editor-json-mvp-increment` | Requirements/Design/Tasks：launcher消费、source journey与packaged journey拆分 | `APPROVED 2026-08-26` |

### Dispatch Authority Register

`dispatch_id`是task使用的稳定amendment身份；`dispatch_request_revision`只在本register内记录同一dispatch的合同版本（`R1`、`R2`），不复制进每个task标签。同一WA在不同dispatch group出现时必须复用该身份、owning Spec acknowledgement与ledger row。真实合同增量必须升revision并把旧request保留为`SUPERSEDED`历史，不能原地改写。当前批准只冻结已acknowledged的R/D/T请求，不预填尚未发生的commit、artifact或终态disposition。owning Spec 的`spec.json`只表达该Spec基线文档/实现状态，不能覆盖本ledger登记的current amendment revision；对Windows cluster，授权条件是“基线允许且current `dispatch_request_revision`为`ACKNOWLEDGED`或更后终态”，任何`PENDING`/`SUPERSEDED`/`BLOCKED`均优先阻断对应suffix task、merge与evidence。

| dispatch_id | dispatch_request_revision | owning_spec | lineage_branch | owner_acknowledgement | current_status |
|---|---|---|---|---|---|
| WA-01 | `R1` | `parser-subsystem-extraction` | `parser-rebaseline` | `APPROVED 2026-08-26` | `ACKNOWLEDGED` |
| WA-02 | `R1` | `collaborative-job-chunks` | `feature/collaborative-job-chunks` | `APPROVED 2026-08-26` | `ACKNOWLEDGED` |
| WA-03 | `R1` | `multi-document-project-workspace` | `feature/multi-document-project-workspace` | `APPROVED 2026-08-26` | `ACKNOWLEDGED` |
| WA-04 | `R1` | `language-resource-portability` | `feature/language-resource-portability` | `APPROVED 2026-08-26` | `ACKNOWLEDGED` |
| WA-05 | `R1` | `tmx-context-interchange` | `ui-mvp` | `APPROVED 2026-08-26` | `ACKNOWLEDGED` |
| WA-06 | `R2` | `tm-storage-retrieval-index` | `feature5` | `APPROVED 2026-08-26` | `ACKNOWLEDGED` |
| WA-07 | `R1` | `feature5-ui-integration` | `ui-mvp` | `APPROVED 2026-08-26` | `ACKNOWLEDGED` |
| WA-08 | `R1` | `qt-editor-json-mvp-increment` | `ui-mvp` | `APPROVED 2026-08-26` | `ACKNOWLEDGED` |

Superseded request history：WA-06 `R1`曾获acknowledgement；Task 0.5发现V1不能表达MIC authority后，由`R2`完整取代并标记`SUPERSEDED`。`R1`不得驱动实现、merge或evidence。
...
| WA-06 | `tm-storage-retrieval-index` / `feature5` | `CONTRACT_CHANGE` | 增加 Windows initial activation、single authority、restart recovery、FTS5 reopen、private attestation、FileId reuse 与 power-cut结果；`R2`以`WindowsPrivateSecurityV2`取代`R1`的V1 physical profile | Core 保留 SQLite authority/generation/reservation/journal/LKG及各自业务 envelope；消费 lock/private/rooted/publish ports，并把绑定V2 security profile/descriptor digest的 W2 `WindowsPrivateProof` 作为子记录嵌入 Gate D/canonical receipt，不把 FileId 当永久身份 | 原`a`任务保持；`9.6a` source completion；`9.6b` frozen revalidation | source：W1 + W2 + ADR-023 + WA-01/02；frozen：W3 spike + platform 7.4 candidate | initial activation/restart/two-process/kill/power-cut；owner envelope + nested V2 proof；DACL/MIC token matrix；tamper/recreate；FTS5/trigram create/query/reopen |
...
[W2 + ADR-023 + WA-01S/02S + Windows backend] -> WA-06S TM Core
...
|---|---|
| dispatch_id | 上表稳定 ID |
| dispatch_request_revision | 与`dispatch_id`组成唯一R/D/T request identity的`R1`/`R2`版本；同一WA跨group不得重发 |
