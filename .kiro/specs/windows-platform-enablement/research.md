...
- candidate-input lock 绑定构建实际可达的 x64 SDK `bin/include/lib`、实际工具解析、producer/runtime/venv 与 pristine build-copy；stock probe 只提供导入表观察。JSON producer 显式使用 UTF-8/LF，否则 Windows 现场生成物与规范化 checkout 会产生非语义字节漂移。当前摘要从 lock 读取，不在 note 另抄一份。
- custom entry 替代 stock 启动路径后，整表继承会把未使用的 C API 和 PE imports 留在目标中。目标采用相对锁定 stock 的显式减法与 custom 增量，绑定签名；实际函数表、导入表及缺失/额外项仍由每次构建独立核对。
...
### Implementation Notes — Task 1.6 原生启动与源码路径

- CPython 的 isolated 初始化仍需随机哈希种子。[锁定版本的 `bootstrap_hash.c`](https://raw.githubusercontent.com/python/cpython/v3.14.7/Python/bootstrap_hash.c) 经 `_Py_HashRandomization_Init → pyurandom → win32_urandom` 调用 `BCryptGenRandom(NULL, ..., BCRYPT_USE_SYSTEM_PREFERRED_RNG)`；应用绑定的是这一[系统 API 合同](https://learn.microsoft.com/en-us/windows/win32/api/bcrypt/nf-bcrypt-bcryptgenrandom)。
- v2 曾把系统 provider 观测快照纳入 candidate 摘要；v3 将构建输入恢复为 ADR-022/023 规定的应用闭包和系统入口策略，主机观测独立保存。这是 W3 输入范围纠偏。
- 分析 native 调用时，缺失私有符号显示的最近导出名不能当作真实函数，指令位移也不等于实参；需核对调用点与寄存器运算。系统查询前后一致只能说明两次观测相同。
- 保留目录句柄与两次 inventory 扫描不能固定“没有新子文件”这一事实。解释器启动源码也须从保留句柄编译、避免磁盘搜索；普通 module 执行 API 可能附带按 CWD 生成的 `SourceFileLoader` metadata。native handoff 必须先于 bootstrap Python 正文，源码 origin 应保持 manifest 身份而非本机工作目录。
- 锁定 MSVC 的 `/Brepro` 本身仍生成 POGO/COFF group 调试目录；`/NOCOFFGRPINFO` 单变量重链接将其移除，保留 REPRO。该选项未列于本机 linker 帮助，维护时须按锁定工具链重新实测最终 PE，不能由 flag 存在推定目录合格。
- E9 的 isolated flags 与空 `module_search_paths` 不等于没有外部路径读取：锁定 runtime 的 native 子进程反例中，`__PYVENV_LAUNCHER__` 仍改写 executable，bundle 父目录的 `pyvenv.cfg` 仍能触发 getpath 读取失败。[版本源码](https://raw.githubusercontent.com/python/cpython/v3.14.7/Modules/getpath.py) 的相关分支未统一受 `use_environment`／显式搜索路径约束。仅在初始化后检查 flags 不能补授这些读取的 authority；消除它们须先复审 W3 的 E9 路径初始化合同，不能把增加 getter、`Py_SetPath` 或关闭 importlib 当作未经验证的替代路线。
- 非空 `Py_SetPath` 可以绕过上述 getpath 配置文件探测，但会先产生非空 `sys.path`；在 `encodings` 适配器才清空仍然太晚。独立诊断在此前显式重放 CPython 异常打印分支，观察到包外 `traceback.py` 正文开始执行，native trace 同时记录其目录枚举、缓存探测和源码路径打开调用。这是失败打印窗口的反例，不是声称自然启动必然触发该错误；[初始化顺序](https://raw.githubusercontent.com/python/cpython/v3.14.7/Python/import.c)与[异常打印器](https://raw.githubusercontent.com/python/cpython/v3.14.7/Python/pythonrun.c)必须一并纳入路径隔离证明。早期 `builtins.open` 尚不可用时，文件 marker 未生成也不能证明正文未执行。
