...
- WA-06 的 Windows schema upgrade 是 C6S 后段的产品前置而非 release-harness 替代项：先由`tm-storage-retrieval-index 5.11a`在已有portable predecessor上闭合same-store-id `N→N+1`、rooted stable backup与fresh recovery，再由`9.6a`闭合固定Windows产品清单、fresh Windows benchmark与C3B source-snapshot handoff；无portable authority的旧SQLite保持零mutation fail closed，不为测试夹具新增bootstrap。通用acceptance/fault/release证据留待Task 9.2由macOS/Linux在同一实现提交上重签，不倒灌POSIX historical-inode语义。只有两者通过后，WA-04/05才可声明active-TM产品汇合并进入Feature5/UI与launcher。
