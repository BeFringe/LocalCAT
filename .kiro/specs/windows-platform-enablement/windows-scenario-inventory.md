...
| exact ACL/token/MIC | 无真实 Windows token matrix | TokenUser SID、owner、V2 exact DACL/control bits、medium label/NO_WRITE_UP、mapped AccessCheck；standard/elevated/local/domain/AzureAD positives 与 low/restricted/service/AppContainer/impersonation negatives | W2 + ADR-023 + WA-06 / 3.3-3.4, 3.6-3.7, 6.3 | `windows.private.tm.token-mic` | `NOT_OBSERVED` |
...
| power-cut/reboot durability | 同上现有 power-cut 仅 fault injection，没有真实 host forced-off/reboot | 与 W1 `DurabilityProfile` 精确绑定的 NTFS/host lane，重启后验证 success boundary 与 portable evidence manifest digest | W1 + WA-03/06 / 3.5-3.7, 5.2, 5.5, 6.1 | `windows.durability.ntfs.forced-reboot` | `NOT_OBSERVED` |
...
2. Tasks 2-3 建立共享合同、POSIX parity 与 Windows W1/W2 backend；没有该层时不得在 consumer 内复制 Win32/POSIX 分支。
