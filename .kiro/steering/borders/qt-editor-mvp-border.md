...
| Step 6 | 修复 QVariant 资源类型边界和设置表格响应布局 | 完成后应能从真实新建对话框创建 TM/术语资源，中文类型与导入操作完整可见 | 待验证 |
| Step 7 | 最近项目、段落断点、退出项目与桌面启动 | 完成后应能从应用菜单启动，在 GUI 内切换/退出项目并恢复上次段落 | 待验证 |
| Step 8 | 左栏紧凑/换行密度和独立浏览校对页 | 完成后应能保持同一会话切换密度与工作区，并从双语浏览行返回同段编辑 | 待验证 |
| Step 9 | `卷一_引.json + OWNattempt.tmx` 真实增量闭环 | 完成后应看到真实 TM 导入统计、项目精确命中、恢复与浏览旅程和全量回归均通过 | 待验证 |
...
- [ ] 使用真实 QComboBox itemData 创建 TM 与术语表，验证 QVariant 边界
- [ ] 使用 `卷一_引.json + OWNattempt.tmx` 从设置导入并得到真实项目 TM 命中
- [ ] 关闭并重开真实项目，验证最近项目、最后段落、密度与工作区模式恢复
- [ ] 在浏览校对页双击长段返回同段编辑，并验证未保存译文未漂移
