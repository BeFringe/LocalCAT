...
- Multi-Document 的后续实现受 ADR-018 约束：ProjectPackage 将作为多文档 workspace 的 canonical persistence，ProjectOrigin 只描述 source/reconciliation/write-back，`codec_private_member` 保持 codec-owned opaque；ProjectPackage 与 ResourcePackage 不共享 authority。当前 runtime 仍是扁平单文档 `EditorProject`，Cluster 0 尚未批准，本文不把 ProjectPackage、目录聚合或 Qt 章节 UI 表述为已实现事实。
