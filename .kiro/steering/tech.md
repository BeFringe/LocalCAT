...
| 层 | 职责 | 当前关键文件 |
|----|------|-------------|
| Layer 1 Storage | legacy JSONL、canonical SQLite、mixed termbase CSV、资源/工作区状态、namespaced Chunk metadata 与持久恢复 | `resource_repository.py`, `workspace_state.py`, `termbase_store.py`, `tm_sqlite_store.py`, `tm_activation_*.py`, `collaborative_chunk_store.py` |
