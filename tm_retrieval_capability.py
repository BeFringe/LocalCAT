...
_PUBLICATION_MEMORY_KEY = object()
_PUBLICATION_REFERENCE_SLOTS: set[MemberDescriptorType] = set()
_PublicationT = TypeVar("_PublicationT")
type _ReferenceAssignment = tuple[MemberDescriptorType, object, object, object]


def _publication_reference_type(name: str, bases: tuple[type, ...], namespace: dict[str, object]) -> type:
    """Create and retain only ordinary Python object-reference slot layouts.

    This memory-layout protocol is not an input/provenance or Gate issuer.
    A native heap type or a claimed __slots__ declaration is never sufficient.
    Returning an ordinary type preserves the existing Core constructor binding.
    """
    if type(name) is not str or type(namespace) is not dict:
        raise TypeError("publication reference type requires fixed Core construction")
    if type(bases) is not tuple or not (not bases or (len(bases) == 1 and bases[0] is object)):
        raise TypeError("publication references require the ordinary object layout")
    slots = namespace.get("__slots__")
    if type(slots) is not tuple or any(type(slot) is not str for slot in slots):
        raise TypeError("publication references require exact tuple slots")
    supplied = tuple(value for value in namespace.values() if type(value) is MemberDescriptorType)
    created = type.__new__(type, name, bases, namespace)
    # These members were generated by the fixed type constructor above,
    # not merely supplied by a caller as a C member or layout declaration.
    for member in vars(created).values():
        if (type(member) is MemberDescriptorType and member.__objclass__ is created
            and not any(member is value for value in supplied)):
            _PUBLICATION_REFERENCE_SLOTS.add(member)
    return created


def _validate_reference_assignments(value: object) -> tuple[_ReferenceAssignment, ...]:
    """Accept only prebuilt exact builtin slot writes, never callbacks."""
    if type(value) is not tuple:
        raise TypeError("publication references must be an exact tuple")
    seen: set[tuple[int, int]] = set()
    for assignment in value:
        if type(assignment) is not tuple or len(assignment) != 4:
            raise TypeError("publication reference must have descriptor/target/prior/new")
        descriptor, target, _prior, _candidate = assignment
        if type(descriptor) is not MemberDescriptorType or type(target) is not descriptor.__objclass__:
            raise TypeError("publication requires an exact builtin slot descriptor")
        if descriptor not in _PUBLICATION_REFERENCE_SLOTS:
            raise TypeError("publication requires a Core-created object reference slot")
        identity = (id(descriptor), id(target))
        if identity in seen:
            raise ValueError("publication cannot write a reference twice")
        seen.add(identity)
    return cast(tuple[_ReferenceAssignment, ...], value)


def _install_publication_reference(assignment: _ReferenceAssignment) -> None:
    descriptor, target, _prior, candidate = assignment
    descriptor.__set__(target, candidate)


@final
class _InputPublicationLifetime:
    """Owner-held memory coordination only; it grants no Gate qualification."""
    __slots__ = ("_lock", "_revoked")
    ...
    def revoke(self) -> None:
        # Scheduler callers release this before any wakeup or native close.
        with self._lock:
            self._revoked = True
    ...
    def window(self, guards: object) -> _InputPublicationWindow:
        return _InputPublicationWindow(self, _validate_reference_assignments(guards), _key=_PUBLICATION_MEMORY_KEY)


@final
class _InputPublicationWindow:
    """Same-owner/epoch slot guards, with no session/native/proof callbacks.

    Only the input owner prepares this after successful terminal/window exit.
    Existing Gate validators remain responsible for production qualification.
    """
    __slots__ = ("__lifetime", "__guards", "__thread", "__prepared", "__committed", "__failed")

    def __init__(self, lifetime: _InputPublicationLifetime, guards: tuple[_ReferenceAssignment, ...], *, _key: object) -> None:
        if _key is not _PUBLICATION_MEMORY_KEY or type(lifetime) is not _InputPublicationLifetime:
            raise TypeError("publication window requires owner memory coordination")
    ...
    def _check_memory(self, *, prepared: bool = False) -> None:
        if (get_ident() != self.__thread or self.__lifetime._revoked
            or self.__committed or self.__failed or (prepared and not self.__prepared)):
            raise RuntimeError("input publication is revoked, incomplete or already committed")
        for descriptor, target, expected, _same in self.__guards:
            if descriptor.__get__(target, type(target)) is not expected:
                raise RuntimeError("input publication owner/epoch/window has changed")

    def prepare(self) -> None:
        with self.__lifetime._lock:
            self._check_memory()
    ...
    def _fail_preparation(self) -> None:
        with self.__lifetime._lock:
            if not self.__committed:
                self.__failed = True

    def plan(self, assignments: object, result: _PublicationT) -> _PreparedInputPublication[_PublicationT]:
        try:
            validated = _validate_reference_assignments(assignments)
            return _PreparedInputPublication(self, validated, result, _key=_PUBLICATION_MEMORY_KEY)
        except BaseException:
            self._fail_preparation()
            raise

    def commit(self, plan: _PreparedInputPublication[_PublicationT], additional: tuple[_ReferenceAssignment, ...] = ()) -> _PublicationT:
        if type(plan) is not _PreparedInputPublication or plan._window is not self:
            raise ValueError("publication plan belongs to a foreign input session")
        assignments = _validate_reference_assignments(plan._assignments + additional)
        # All original observer/publisher locks are already held. Keep both old
        # and new references alive until this innermost mutex has been released.
        with self.__lifetime._lock:
            self._check_memory(prepared=True)
            for descriptor, target, prior, _candidate in assignments:
                if descriptor.__get__(target, type(target)) is not prior:
                    raise RuntimeError("publication reference changed before commit")
            try:
                for assignment in assignments:
                    _install_publication_reference(assignment)
                self.__committed = True
            except BaseException as install_error:
                self.__failed = True
                restore_errors: list[BaseException] = []
                for descriptor, target, prior, _candidate in reversed(assignments):
                    try:
                        descriptor.__set__(target, prior)
                    except BaseException as restore_error:
                        restore_errors.append(restore_error)
                if restore_errors:
                    self.__lifetime._revoked = True
                    raise BaseExceptionGroup("publication installation and rollback failed", [install_error, *restore_errors]) from None
                raise
        return plan._result

    def commit_value(self, value: _PublicationT | _PreparedInputPublication[_PublicationT], additional: tuple[_ReferenceAssignment, ...]) -> _PublicationT:
        plan = value if type(value) is _PreparedInputPublication else self.plan((), cast(_PublicationT, value))
        return self.commit(plan, additional)


@final
class _PreparedInputPublication(Generic[_PublicationT]):
    """Immutable prebuilt references; the caller retains its observation locks."""
    __slots__ = ("_window", "_assignments", "_result")

    def __init__(self, window: _InputPublicationWindow, assignments: tuple[_ReferenceAssignment, ...], result: _PublicationT, *, _key: object) -> None:
        if _key is not _PUBLICATION_MEMORY_KEY:
            raise TypeError("publication plan requires owner memory coordination")
        self._window = window
        self._assignments = assignments
        self._result = result

    def __setattr__(self, name: str, value: object) -> None:
        if name in ("_window", "_assignments", "_result") and not hasattr(self, name):
            object.__setattr__(self, name, value)
            return
        raise AttributeError("publication plans are immutable")

    def __delattr__(self, name: str) -> None:
        raise AttributeError("publication plans are immutable")

    def commit(self) -> _PublicationT:
        return self._window.commit(self)
...
@final
class RetrievalCapabilityPublisher(metaclass=_publication_reference_type):
    ...
    def _validated_transition(
        ...
        *,
        ...
        _publication_window: _InputPublicationWindow | None = None,
    ) -> _ValidatedPublicationT:
        ...
        if _publication_window is not None and type(_publication_window) is not _InputPublicationWindow:
            raise TypeError("publisher publication window must be exact")
        instant = _require_utc_instant(evaluated_at_utc)
        ...
        with self.__lock:
            ...
            prior = snapshot_get(self, RetrievalCapabilityPublisher)
            prepared = prepare(next_snapshot)
            if _publication_window is not None:
                return _publication_window.commit_value(prepared, ((
                    _snapshot_descriptor, self, prior, next_snapshot,
                ),))
...
def _validated_refresh_retrieval_capability(
    ...
    *,
    ...
    return publisher._validated_transition(
        ...
        _publication_window=_publication_window,
    )
